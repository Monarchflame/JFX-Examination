...
import ldh.fx.transition.FadeInUpTransition;
import ldh.fx.transition.FadeOutDownTransition;
...
public class LNavPane extends ScrollPane {
    ...
    private void addNode(Node node) {
        ...
        for (Node nodet : nodeList) {
            if (nodet instanceof Button) {
                ...
                child.addEventHandler(ActionEvent.ACTION, e->{
                ...
                for (Node node2 : pane.getChildren()) {
                    if (node2 instanceof Button) {
                        node2.addEventHandler(ActionEvent.ACTION, e->{
                            removeAllActiveClass();
                            addActiveClass(node2);
                        });
    ...
    private void addActiveClass(Node child) {
        child.getStyleClass().add("active-nav");
    }

    private void removeAllActiveClass() {
        Pane bigLeftPane = (Pane) stackPane.getChildren().get(1);
        for (Node node : bigLeftPane.getChildren()) {
            if (node instanceof Button) {
                node.getStyleClass().remove("active-nav");
            } else if (node instanceof TitledPane) {
                Pane pane = (Pane) ((TitledPane) node).getContent();
                for (Node n2 : pane.getChildren()) {
                    if (n2 instanceof Button) {
                        n2.getStyleClass().remove("active-nav");
                    }
                }
            }
        }
    ...
    private Popup buildPopup(Button b, TitledPane titledPane) {
        ...
        popup.showingProperty().addListener((ob, o, n) -> {
            if (n) {
                ...
                FadeInUpTransition bounceInTransition = new FadeInUpTransition(box);
                bounceInTransition.play();
            } else {
                b.getStyleClass().remove("nav-show");
                FadeOutDownTransition fadeOutDownTransition = new FadeOutDownTransition(box);
                fadeOutDownTransition.play();
            }
        });
        return popup;
    }
