...
public class HomeHeadController implements Initializable {
    ...
    @FXML public void settingBtn(ActionEvent e) {
        LDialog ldhDialog = new LDialog(PrimaryStage.STAGE, "test", 500d, 600d, DialogModel.Normal);
        VBox box = new VBox();
        for (int i=0; i<10; i++) {
    ...
    @FXML public void profileBtn(ActionEvent actionEvent) {
        LDialog ldhDialog = new LDialog(PrimaryStage.STAGE, "test", 500d, 300d, DialogModel.Normal);
        VBox box = new VBox();
        for (int i=0; i<10; i++) {
            box.getChildren().add(new Label("asdfasfd"));
        }
