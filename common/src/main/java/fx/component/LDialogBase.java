...
//            isShowingMinButton(false);
    ...
    private void stageMax(ActionEvent evt) {
        ...
        Screen screen = getScreenByWindow(w);
    ...
    private Screen getScreenByWindow(Window w) {
        ObservableList<Screen>  screens = Screen.getScreensForRectangle(w.getX(), w.getY(), w.getWidth(), w.getHeight());
        if (screens.size() >= 1) {
