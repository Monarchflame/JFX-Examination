...
import fx.ui.util.RegionResizable;
import fx.ui.util.Resizable;
import fx.ui.util.StageMovable;
import fx.ui.util.StageResizable;
...
public class LWindowBase extends AnchorPane {
    ...
    @FXML protected VBox dialogContainer;
    ...
    public LWindowBase() {
        ...
        topProperty.addListener((l, o, n)->{
            if (o != null && dialogContainer.getChildren().contains(o)) {
                dialogContainer.getChildren().remove(o);
            }
            dialogContainer.getChildren().add(0, n);
        });
    }
    ...
    public void setContentPane(Node node) {
        centerProperty.set(node);
        node.setStyle("-fx-border-radius: 10px;\n" +
